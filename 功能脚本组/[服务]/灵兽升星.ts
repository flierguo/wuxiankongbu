...
export function Main(Npc: TNormNpc, Player: TPlayObject, Args: TArgs): void {
    ...
    const S = `\\\\\\\\\\
                         灵兽升星\\\\
        {S=升星属性:根据灵兽的品质按照比例增加攻击力;C=251}\\
        {S=升星要求:需要【等级*10】的血脉精玉;C=191}\\
        {S=星套属性:全套生肖+马牌总数值达到一定数量触发星套属性;C=254}\\\\
...
`
    Npc.SayEx(Player, 'NPC小窗口带2框', S)  // <一键升星/@@InPutInteger(可设置升星次数输入5-16的数值，一键升星的次数=升星石数量)>
}
export function 升星(Npc: TNormNpc, Player: TPlayObject, Args: TArgs): void {
    ...
    let 几率 = random(100)
    let 装备 = Player.GetCustomItem(0)
    let 血脉精玉 = Player.GetCustomItem(1)
    let 升星石数量 = (装备.GetOutWay3(40) + 1) * 10
    let 数值 ='0'
    if (装备 == null || 装备.StdMode != 35) { Player.MessageBox(`请放入马牌再进行升星!`); return }
    if (血脉精玉 == null || 血脉精玉.GetName() != '血脉精玉'|| 血脉精玉.GetDura() < 升星石数量) { Player.MessageBox(`请放入血脉精玉或血脉精玉数量不足!当前需求数量: ${升星石数量}`); return }
    ...
    if (random(100) < 升星几率) {
        数值 = js_number(装备生肖[装备生肖.length-1], js_number(生肖数值, 灵兽升星数值比例[装备.GetOutWay3(40)], 3), 1)
        ...
        Npc.Take(Player, 血脉精玉.GetName(), 2)
