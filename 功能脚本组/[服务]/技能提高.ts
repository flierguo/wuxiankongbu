...
export function Main(Npc: TNormNpc, Player: TPlayObject, Args: TArgs): void {
    ...
    switch (true) {
        case Player.V.战神:
            ...
            M += 显示技能提升选项(Player, ["狂怒", "召唤战神", "战神附体", "天神附体", "万剑归宗"])
            break
        case Player.V.骑士:
            ...
            M += 显示技能提升选项(Player, ["圣光打击", "防御姿态", "惩罚", "愤怒", "审判救赎"])
            break
        case Player.V.火神:
            ...
            M += 显示技能提升选项(Player, ["法术奥义", "闪现", "致命一击", "火墙之术", "群星火雨"])
            break
        case Player.V.冰法:
            ...
            M += 显示技能提升选项(Player, ["暴风雨", "魔法盾", "打击符文", "冰霜之环", "寒冬领域"])
            break
        case Player.V.驯兽师:
            ...
            M += 显示技能提升选项(Player, ["萌萌浣熊", "凶猛野兽", "嗜血狼人", "丛林虎王", "雷暴之怒"])
            break
        case Player.V.牧师:
            ...
            M += 显示技能提升选项(Player, ["剧毒火海", "妙手回春", "互相伤害", "恶魔附体", "末日降临"])
            break
        case Player.V.刺客:
            ...
            M += 显示技能提升选项(Player, ["潜行", "弱点", "致命打击", "增伤", "暗影杀阵"])
            break
        case Player.V.鬼舞者:
            ...
            M += 显示技能提升选项(Player, ["鬼舞斩", "鬼舞术", "鬼舞之殇", "鬼舞者", "群魔乱舞"])
            break
        case Player.V.神射手:
            ...
            M += 显示技能提升选项(Player, ["复仇", "成长", "生存", "万箭齐发", "神灵救赎"])
            break
        case Player.V.猎人:
            ...
            M += 显示技能提升选项(Player, ["分裂箭", "召唤宠物", "宠物突变", "人宠合一", "命运刹印"])
            break
        case Player.V.武僧:
            ...
            M += 显示技能提升选项(Player, ["天雷阵", "护法灭魔", "体质强化", "至高武术", "碎石破空"])
            break
        case Player.V.罗汉:
            ...
            M += 显示技能提升选项(Player, ["金刚护法", "转生", "金刚护体", "擒龙功", "轮回之道"])
            break
    }
    Npc.SayEx(Player, 'NPC中窗口', M)

    // if (Player.GetRightHand){Debug(`武器名字:${Player.RightHand.DisplayName} + 经验倍率:${Player.R.经验加成}`)}
    
}

// 显示技能提升选项函数
function 显示技能提升选项(Player: TPlayObject, 技能列表: string[]): string {
    let 结果 = `\\\\\\{S=技能提升;C=250;X=50}\\\\`
    
    for (const 技能名称 of 技能列表) {
        const 变量名 = 技能名称 + "等级"
        const 技能描述 = Player.V[变量名] + Player.R[变量名] + Player.R.所有技能等级 || 0
        const 当前等级 = Player.V[变量名] || 0
        const 下一级 = 当前等级 + 1
        const 需要书页 = 500 * 下一级
        
        结果 += `{S=${技能名称};C=154;X=50}  {S=当前等级:${技能描述};X=250}  <{S=提升;X=400;HINT=需要书页${需要书页}个}/@提升技能(${技能名称})>\\\\`
    }
...
// 提升技能等级函数
export function 提升技能(Npc: TNormNpc, Player: TPlayObject, Args: TArgs): void {
    const 技能名称 = Args.Str[0]
    const 变量名 = 技能名称 + "等级"
    const 当前等级 = Player.V[变量名] || 0
    const 下一级 = 当前等级 + 1
    const 需要书页 = 500 * 下一级
    
    if (Player.GetItemCount('书页') < 需要书页) {
        Player.MessageBox(`提升${技能名称}到${下一级}级需要${需要书页}个书页，你的书页不足！`)
        return
    }
    
    Npc.Take(Player, '书页', 需要书页)
    Player.V[变量名] = 下一级
