...
export function fTime(): string {
    ...
    const 年月日都在 = year + '-' + mon + '-' + date + ' ' + hours + ':' + minutes + ':' + seconds;
...
//             // GameLib.V.时间[Player.GetName()][数据库ID]['小时'] ??= 0
//             // GameLib.V.时间[Player.GetName()][数据库ID]['分钟'] ??= 0
//             // GameLib.V.时间[Player.GetName()][数据库ID]['秒钟'] ??= 0
//             // if (DateUtils.DaysBetween(hours, GameLib.V.时间[Player.GetName()][数据库ID]['小时']) > 0) { }
//             if (显示类型 == g.出售列表) {
//                 switch (订单状态) {
//                     case g.出售中: 换算 = "出售中"; 换算颜色 = 255; 显示文字 = "取回"; break;
//                     case g.已售出: 换算 = `已售出`; 换算颜色 = 250; 显示文字 = "取出元宝"; break;
...
//                 if (订单状态 == g.已售出) {
//                     str += `{s=(收益：${Math.ceil(价格 * 0.95)} 点券);x=630;y=${y};c=250}`
//                 }
//             } else {
...
// }
// export function 取出元宝(Npc: TNormNpc, Player: TPlayObject, Args: TArgs): void {
...
//         Player.SendMessage('点券不足' + 出售价格 + ", 无法购买")
//         return
//     }
...
//                 取回人.SendMessage('成功取出: ' + 点券 + "元宝, 祝老板生意兴隆, 发大财!", 1)
//                 return true
//             } else {
//                 return false
//             }
...
//     if (Player.GetLevel() < 200) { Player.MessageBox('等级低于200级不允许出售物品'); return }
//     let 放入的物品 = Player.GetCustomItem(0)
//     let 出售价格 = Args.Int[0]
//     let 手续费 = Player.V[g.出售计数] <= 5 || Player.V[g.出售计数] == undefined ? 0 : 5
//     if (放入的物品 == undefined) {
...
//     if (是否成功 > 0) {
//         Player.SendMessage(`{s=你以 }{s=${出售价格}元宝;c=249}{s=的价格上架了}{s=${数量}个;}{s=${放入的物品.Name};c=${放入的物品.Color}}`, 1)
