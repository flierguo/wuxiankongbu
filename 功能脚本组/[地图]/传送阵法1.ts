...
export function Main(Npc: TNormNpc, Player: TPlayObject, Args: TArgs): void {
    ...
    {S=当前倍数: ${Player.V.挑战倍数} 倍;X=50;Y=360}          
...
export function 副本传送(Npc: TNormNpc, Player: TPlayObject, Args: TArgs): void {
    ...
    for (let i = 1; i <= 固定副本数; i++) {
        ...
        if (副本 && 副本.地图ID && 副本.地图ID !== '') {
            ...
            // 固定副本HINT：创建者为系统自建，剩余时间为无限
            let hint = `HINT=当前地图:{S=${副本.显示名};C=146}#92地图人数:{S=${人数} 人;C=251}#92创建者:{S=系统自建;C=255}#92剩余时间:{S=无限;C=253}`;
            str += `<&{S=${副本.显示名};FS=11;C=103;X=80;Y=${y};${hint}}/@固定传送(${地图名},${下标})>`;
        } else {
    ...
    for (let i = 固定副本数 + 1; i <= 9; i++) {
        ...
        if (副本 && 副本.地图ID && 副本.地图ID !== '' && 副本.是圣耀副本) {
            ...
            let 创建者名 = '未知';
            if (副本.创建者ID) {
                let 创建者 = GameLib.FindPlayer(副本.创建者ID);
                创建者名 = 创建者 ? 创建者.GetName() : `玩家${副本.创建者ID}`;
            }
            
            // 圣耀副本HINT
            let hint = `HINT=当前地图:{S=${副本.显示名};C=146}#92地图人数:{S=${人数} 人;C=251}#92创建者:{S=${创建者名};C=255}#92剩余时间:{S=${剩余时间};C=253}`;
            str += `<&{S=${副本.显示名};FS=11;C=250;X=80;Y=${y};${hint}}/@圣耀副本传送(${下标})>`;
        } else {
            str += `<&{S=创建圣耀副本;FS=11;C=154;X=80;Y=${y}}/@创建圣耀副本(${地图名})>`;
        }
    }
