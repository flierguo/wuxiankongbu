...
export interface 地图配置项 {
    ...
    固定星级: number       // 基础星级
    需求等级: number       // 进入等级要求
    地图等级: number       // 地图难度等级（锚点计算基础）
    地图宽度?: number      // 可选，不填则自动获取
    地图高度?: number      // 可选，不填则自动获取
    // 锚点配置（可微调，不填则自动计算）
    锚点?: {
        生物强度倍数?: string    // 大数值字符串，覆盖自动计算
        经验倍数?: number        // 覆盖自动计算
        爆率倍数?: number        // 覆盖自动计算
    }
}
...
// ==================== 地图配置表 ====================
// 锚点配置说明：
// - 生物强度倍数：大数值字符串，用于乘以怪物基础属性
// - 经验倍数/爆率倍数：可选微调，不填则根据地图等级自动计算
export const 地图配置: 地图配置项[] = [
    { 地图名: 地图名称.浣熊市,     下标: 1,   固定星级: 1,   需求等级: 1,   地图等级: 100,  锚点: { 生物强度倍数: '1',    经验倍数: 1,  爆率倍数: 1 } },
    { 地图名: 地图名称.蜂巢入口,   下标: 11,  固定星级: 2,   需求等级: 50,  地图等级: 200,  锚点: { 生物强度倍数: '2',    经验倍数: 2,  爆率倍数: 1 } },
    { 地图名: 地图名称.病毒研究室, 下标: 21,  固定星级: 3,   需求等级: 100, 地图等级: 300,  锚点: { 生物强度倍数: '4',    经验倍数: 3,  爆率倍数: 2 } },
    { 地图名: 地图名称.生化实验室, 下标: 31,  固定星级: 4,   需求等级: 150, 地图等级: 400,  锚点: { 生物强度倍数: '8',    经验倍数: 4,  爆率倍数: 2 } },
    { 地图名: 地图名称.激光走廊,   下标: 41,  固定星级: 5,   需求等级: 200, 地图等级: 500,  锚点: { 生物强度倍数: '16',   经验倍数: 5,  爆率倍数: 3 } },
    { 地图名: 地图名称.红后控制室, 下标: 51,  固定星级: 6,   需求等级: 250, 地图等级: 600,  锚点: { 生物强度倍数: '32',   经验倍数: 6,  爆率倍数: 3 } },
]
 ...
 * @param 自定义锚点 可选的自定义锚点配置
 */
export function 计算锚点(地图等级: number, 难度倍数: number = 1, 自定义锚点?: 地图配置项['锚点']): 锚点配置 {
    ...
    let 经验倍数 = 实际锚点
    let 爆率倍数 = Math.max(1, Math.floor(实际锚点 / 2))
    
    // 应用自定义锚点配置（微调）
    if (自定义锚点) {
        if (自定义锚点.生物强度倍数) {
            // 自定义倍数 * 难度倍数
            生物强度倍数 = 智能计算(自定义锚点.生物强度倍数, String(难度倍数), 3)
        }
        if (自定义锚点.经验倍数 !== undefined) {
            经验倍数 = 自定义锚点.经验倍数 * 难度倍数
        }
        if (自定义锚点.爆率倍数 !== undefined) {
            爆率倍数 = 自定义锚点.爆率倍数
        }
    }
