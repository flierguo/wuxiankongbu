...
const 数值平衡配置 = {
    ...
    防御倍数最大: 5,         // 防御随机倍数最大值（原20）
    魔次抵抗系数: 10000,     // 魔次抵抗计算系数：防御/系数
    魔次抵抗上限倍数: 5,     // 魔次抵抗上限 = 地图等级 * 倍数
} as const
...
export function Refresh(Envir: TEnvirnoment, Monster: TActor, Tag: number): void {
    ...
    // 魔次抵抗上限控制（避免高等级地图魔次过高）
    const 魔次抵抗上限 = 智能计算(String(地图信息.地图等级), String(数值平衡配置.魔次抵抗上限倍数), 3)
    if (比较(魔次抵抗, 魔次抵抗上限) > 0) {
        魔次抵抗 = 魔次抵抗上限
    }

    // 经验值计算（使用地图等级 * 经验倍数 * 变异倍数）
    let 经验值 = 地图信息.地图等级 * 配置.经验倍数 * 变异倍数
