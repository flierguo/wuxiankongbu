...
import { 取完整地图配置, 难度系数配置, 装备基础值配置 } from '../世界配置'
import { TAG, 基础词条, 技能魔次, 装备颜色, 基础属性分割, 基础属性第一条, 基础属性第八条, 职业魔次分割, 职业第一条, 职业第六条, 装备需求等级, 基础数值, 怪物装备倍数 } from '../基础常量'
...
/**
 * 装备属性倍数配置
 * 简化后只保留核心配置，移除惊喜掉落等冗余配置
 * 新公式: 属性 = 基础值 × 地图难度倍数 × 随机倍数(0.3-1.7) × 极品倍率
 */
const 数值平衡配置 = {
    随机倍数最小: 0.3,       // 随机倍数最小值（扩大范围增加多样性）
    随机倍数最大: 1.7,       // 随机倍数最大值（扩大范围增加多样性）
    基础翻倍最小: 50,        // 基础翻倍最小值
    基础翻倍范围: 200,       // 基础翻倍范围
} as const
...
/**
 * 计算属性基础值（每条属性独立随机）
 * 新公式: 属性 = 基础值 × 地图难度倍数 × 随机整数因子
 * 
 * @param 地图强度 地图基础强度
 * @param 难度名 难度名称（简单/普通/困难/精英/炼狱/圣耀）
 * @param 极品倍率 统一的极品倍率（由主函数计算）
 * @param 神器倍数 神器倍数（仅神器生效）
 * @param 装备倍数 装备倍数怪的倍数（0表示普通怪）
 * @param 调试玩家 可选的玩家对象，用于输出调试信息
 */
function 计算属性值(地图强度: number, 难度名: string, 极品倍率: number, 神器倍数: number, 装备倍数: number = 0, 调试玩家?: TPlayObject): string {
    // 1. 获取难度系数（从世界配置导入）
    const 难度系数 = 难度系数配置[难度名 as keyof typeof 难度系数配置] || 1
    
    // 2. 计算地图难度倍数 = 地图强度 × 难度系数
    const 地图难度倍数 = 地图强度 * 难度系数
    
    // 3. 生成随机整数因子（50-150范围，确保每条属性有明显差异）
    const 随机整数因子 = 50 + random(100)
    
    // 4. 计算总倍数（不除以100，直接使用整数因子）
    // 总倍数 = 地图难度倍数 × 随机整数因子 × 极品倍率 × 神器倍数 × 装备倍数
    let 总倍数 = 地图难度倍数 * 随机整数因子 * 随机小数(0.05,0.2)
    if (极品倍率 > 1) {
        总倍数 *= 极品倍率
    }
    if (神器倍数 > 1) {
    ...
    // 5. 一次性计算最终属性值（仅1次智能计算调用）
    const 属性值 = 智能计算(String(装备基础值配置.属性基础), String(总倍数), 3)
    
    // 调试输出
    if (调试玩家) {
        console.log(`[调试] 地图强度:${地图强度} 整数因子:${随机整数因子} 总倍数:${总倍数} 属性值:${属性值}`)
    }
...
/**
 * 计算魔次属性值（每条属性独立随机）
 * 新公式: 属性 = 魔次基础值 × 地图难度倍数 × 随机整数因子 × 极品倍率
 * 
 * @param 地图强度 地图基础强度
 * @param 难度名 难度名称
 ...
 * @param 神器倍数 神器倍数（仅神器生效）
 */
function 计算魔次属性值(地图强度: number, 难度名: string, 分组: number, 极品倍率: number, 神器倍数: number): string {
    // 1. 获取难度系数（从世界配置导入）
    const 难度系数 = 难度系数配置[难度名 as keyof typeof 难度系数配置] || 1
    ...
    // 4. 计算总倍数
    // 职业技能(分组2)数值为其他的1/10000，其他分组为1/100
    const 分组系数 = (分组 === 2) ? 0.0001 : 0.01
    
    let 总倍数 = 地图难度倍数 * 随机整数因子 * 分组系数
    if (极品倍率 > 1) {
        总倍数 *= 极品倍率
    }
    if (神器倍数 > 1) {
    ...
    // 5. 一次性计算最终属性值（仅1次智能计算调用）
    let 属性值 = 智能计算(String(装备基础值配置.魔次基础), 总倍数.toFixed(6), 3)
...
export function 装备掉落(Player: TPlayObject, Monster: TActor, UserItem: TUserItem): void {
    ...
    // 获取难度名（默认为简单）
    const 难度名 = '简单'  // TODO: 后续可从地图配置或玩家选择获取
    ...
    const 开启调试 = false
    if (开启调试) {
        console.log(`[装备调试] 开始生成 ${基础词条数} 条基础词条`, 1)
    }
    ...
    while (已生成词条数 < 基础词条数 && 当前索引 <= 基础属性第八条 && 尝试次数 < 最大尝试次数) {
        ...
        if (!已用基础词条.includes(词条类型)) {
            ...
            const 属性值 = 计算属性值(地图强度, 难度名, 基础极品倍率, 神器倍数, 装备倍数, 开启调试 ? Player : undefined)
    ...
    for (let i = 0; i < 魔次词条数 && 魔次索引 <= 职业第六条; i++) {
        ...
        const 属性值 = 计算魔次属性值(地图强度, 难度名, 分组, 技能极品倍率, 神器倍数)
