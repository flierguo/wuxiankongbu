...
// 重新导出供其他模块使用
export { 难度系数配置, 难度列表, 固定难度列表, 固定副本数量, 完整地图配置 }
...
export function 初始化副本池() {
    ...
    if (现有副本池 && 现有副本池.length > 0) {
        ...
        for (const 配置 of 完整地图配置) {
            let 检测下标 = 配置.下标
            if (现有副本池[检测下标]?.地图ID && 现有副本池[检测下标].地图ID !== '') {
                已有固定副本 = true
                break
            }
        }
    ...
    // 初始化副本池数组（稀疏数组，用配置下标作为起始位置）
    // 每个地图占用 最大副本数(10) 个槽位：下标+0 保留，下标+1~6 固定难度，下标+7~9 圣耀
    let 副本池: 副本信息[] = []
    ...
    // 为每个地图创建固定难度副本（使用配置中的下标作为起始位置，无保留位）
    // 例：浣熊市下标101，简单=101，普通=102，困难=103，精英=104，炼狱=105，圣耀位=106-110
    完整地图配置.forEach((配置) => {
        let 起始下标 = 配置.下标
        固定难度列表.forEach((难度名, 难度索引) => {
            let 下标 = 起始下标 + 难度索引
            创建固定副本(配置.地图名, 下标, 配置.固定星级, 配置.需求等级, 难度名)
        })
    })
}
...
export function 创建圣耀副本(地图名: string, Player: TPlayObject): number {
    ...
    for (let i = 起始下标 + 固定副本数量; i < 起始下标 + 最大副本数; i++) {
    ...
    for (let i = 0; i < 副本池.length; i++) {
        ...
        if (玩家数量 < 1 && GameLib.R[`副本_${i}_检测清理`] < 30 && 已激活) {
            ...
            if (GameLib.R[`副本_${i}_检测清理`] >= 10) {
                // 判断是否为固定副本：找到该下标所属地图的起始下标，计算偏移量
                let 是固定副本 = false
                for (const 配置 of 完整地图配置) {
                    let 偏移 = i - 配置.下标
                    if (偏移 >= 0 && 偏移 < 固定副本数量) {
