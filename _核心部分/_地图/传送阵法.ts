...
export function 副本传送(Npc: TNormNpc, Player: TPlayObject, Args: TArgs): void {
    ...
    for (let i = 1; i <= 固定副本数; i++) {
        ...
        if (副本 && 副本.地图ID && 副本.地图ID !== '') {
            ...
            str += `{S=${副本.显示名};FS=11;C=103;X=80;Y=${y}}<{M=297,297,297;F=新UI素材文件.data;X=28;Y=${y - 12};HINT=${hint}}/@固定传送(${地图名},${下标})>`;
        } else {
    ...
    for (let i = 固定副本数 + 1; i <= 10; i++) {
        ...
        if (副本 && 副本.地图ID && 副本.地图ID !== '' && 副本.是圣耀副本) {
            ...
            str += `{S=${副本.显示名};FS=11;C=250;X=80;Y=${y}}<{M=297,297,297;F=新UI素材文件.data;X=28;Y=${y - 12};HINT=${hint}}/@圣耀副本传送(${下标})>`;
        } else {
            str += `{S=创建圣耀副本;FS=11;C=154;X=80;Y=${y}}<{M=297,297,297;F=新UI素材文件.data;X=28;Y=${y - 12};HINT=点击创建圣耀副本}/@创建圣耀副本(${地图名})>`;
        }
    }
