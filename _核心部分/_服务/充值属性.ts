 ...
 */
const 礼包配置 = [
    { 额度: 10, 爆率: 0, 回收: 0, 极品: 0, 鞭尸: 0, 特殊: '自动回收 + 材料存仓' },
    ...
    { 额度: 600, 爆率: 300, 回收: 300, 极品: 140, 鞭尸: 0, 特殊: '' },
    { 额度: 700, 爆率: 350, 回收: 350, 极品: 160, 鞭尸: 0, 特殊: '' },
    { 额度: 800, 爆率: 400, 回收: 400, 极品: 180, 鞭尸: 0, 特殊: '' },
    { 额度: 900, 爆率: 450, 回收: 450, 极品: 180, 鞭尸: 2, 特殊: '' },
    { 额度: 1000, 爆率: 500, 回收: 500, 极品: 180, 鞭尸: 4, 特殊: '' },
];
...
export function Main(Npc: TNormNpc, Player: TPlayObject, Args: TArgs): void {
    ...
    let Str = `{S=赞助礼包中心;C=254;X=180;Y=38}\\\\`;
    Str += `{S=当前充值金额:${Player.V.真实充值 || 0}R;C=150;X=40;Y=60}\\\\`;
    ...
    for (let i = 0; i < 礼包配置.length; i++) {
        ...
        const 状态文字 = 已领取 ? '{S=已领取;C=249}' : `<{S=领取;C=250}/@领取礼包(${i})>`;
        ...
        Str += `{S=充值满${配置.额度}R: ${福利描述} +${主神点}主神点;C=2;Y=${Y}} ${状态文字}\\`;
        Y += 22;
    }

    Str += `\\{S=充值比例: 1R = 1充值金额;C=224;X=40;Y=${Y + 10}}`;
...
export function 领取礼包(Npc: TNormNpc, Player: TPlayObject, Args: TArgs): void {
    ...
    if (配置.特殊) {
        ...
        Player.V.自动回收 = true;
        Player.V.材料入仓 = true;
        Player.MessageBox(`恭喜领取${额度}R礼包！获得：自动回收、材料存仓功能 + ${主神点}主神点`);
    } else {
        // 累加赞助属性
        Player.V.赞助爆率 = (Player.V.赞助爆率 || 0) + 配置.爆率;
        Player.V.赞助回收 = (Player.V.赞助回收 || 0) + 配置.回收;
        Player.V.赞助极品 = (Player.V.赞助极品 || 0) + 配置.极品;
        Player.V.赞助鞭尸 = (Player.V.赞助鞭尸 || 0) + 配置.鞭尸;
...
export function DoObtainGold(Npc: TNormNpc, Player: TPlayObject, Args: TArgs): void {
    ...
    try {
        ...
        if (数量 > 0) {
            Player.V.累计领取次数 ??= 0;
            主神点 = Math.floor(数量 * 10); // 额度 * 10 的主神点
