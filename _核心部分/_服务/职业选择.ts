...
const 职业数据 = {
    ...
    血神: {
        技能: [
            { 图标: 822, 名称: '血气献祭', 描述: '主动,对目标造成500%的伤害,每级提高50%' },
    ...
    暗影: {
        技能: [
            { 图标: 890, 名称: '暗影猎取', 描述: '被动,攻击1%的几率获取暗影值,每点提高1%主属性,每5级提高1点上限.' },
            { 图标: 900, 名称: '暗影袭杀', 描述: '主动,对目标造成500%伤害,每级提高50%.' },
            { 图标: 910, 名称: '暗影剔骨', 描述: '开启后,对周围6码范围内敌人造成300%伤害,每级提高30%,每秒消耗1点暗影值.' },
            { 图标: 940, 名称: '暗影风暴', 描述: '主动,对目标范围5码内敌人造成400%伤害,消耗5点暗影值.每级提高40%.冷却20S.' },
            { 图标: 942, 名称: '暗影附体', 描述: '主动,消耗10点暗影值,使暗影袭杀的攻击提高至1000%,持续5S,每10级增加1S.冷却3分钟' },
        ]
    },
    烈焰: {
        技能: [
    ...
    正义: {
        技能: [
            ...
            { 图标: 684, 名称: '审判', 描述: '被动,对满血目标造成5%血量切割，每200重+1%切割,封顶20%.' },
            { 图标: 668, 名称: '神罚', 描述: '被动,攻击1%几率使目标遭受神罚，损失当前生命的1%，每200级+1%,封顶20%.' },
        ]
    },
    不动: {
        技能: [
            ...
            { 图标: 978, 名称: '铁布衫', 描述: '被动,提供5%格挡，每50级提高1%（50上限）' },
...
// 新人职业技能 - 根据当前Job学习对应职业技能
export function 新人职业技能(Player: TPlayObject): void {
    // 获取玩家当前Job
    const jobId = Player.Job
    const 本职业 = 本职业数据[jobId]

    if (!本职业) {
        Player.MessageBox('无效的职业!');
        return
    }
    ...
    // 添加当前Job的本职业技能
    const 当前职业技能 = 获取本职业技能列表(jobId)
    for (const 技能名 of 当前职业技能) {
        Player.AddSkill(技能名, 1)
    }
